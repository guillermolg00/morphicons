...
```bash
bun test                   # full suite (135 tests) — always green
bun run typecheck          # strict ×5: root (core+dom WITHOUT lib DOM), playground, react, vue, svelte
bunx biome check --write . # lint + format
bun run build              # tsdown (entries index, dom, react, vue, svelte-shared; shared core
                           # chunk) + scripts/build-svelte.ts (dist/svelte.{js,d.ts} + MorphIcon.svelte)
bun run size               # size gates (7 / 7.5 / 8.5 / 8.5 / 8 / 1.4 KB gzip)
...
- **Zero runtime dependencies**. None. devDeps yes; `react` (>= 18), `vue` (>= 3.3) and `svelte` (>= 5) are **optional** peers, only for `./react`, `./vue` and `./svelte`.
- **The core never touches the DOM**: pure functions consume icon data and produce `d` strings and numbers. DOM only in `src/dom` (via ambient declares — compiles without `lib: DOM`); React only in `src/react`; Vue only in `src/vue`; Svelte only in `src/svelte`. The **root** tsconfig deliberately omits `lib: DOM` and excludes `src/react` + `test/react` + `src/vue` + `test/vue` + `src/svelte` + `test/svelte`; the playground/react/vue/svelte/build tsconfigs add DOM (and jsx where needed). `tsconfig.build.json` exists ONLY so tsdown can emit types — the separation is enforced by `bun run typecheck`.
- ESM only, TypeScript strict, `sideEffects: false`, subpath exports (`.`, `./dom`, `./react`, `./vue`, `./svelte`).
...
- **The three bindings share ONE lifecycle contract** (README "Lifecycle contract"): lazy driver on iconless mounts (the first icon — prop or imperative — births it; `morphTo` without driver ≡ `set`), controlled wins over `icon` while `from`/`to` are both present, and every exit from controlled mode invalidates the frozen pair. No fix or behavior change may land in a single binding — the mount suites are mirrored on purpose.
...
- The driver's `PathEl` is structural (`{ setAttribute }`): the dom driver tests use a fake element and a rAF injected into globalThis — the driver is never handed a real element there, which is what keeps `src/dom` honest about reading globals at runtime. Keep it that way. The BINDING suites are hybrid on purpose: logic with fakes where it is extracted (`test/svelte/controller.test.ts`) + real mounts on **happy-dom** for the framework wiring (`test/*/mount.*`). happy-dom is a devDep preloaded for the whole run (`test/register-client-dom.ts` — vue captures `document` at import time); the deterministic rAF fake in `test/client-dom.ts` drives all client frames. No jsdom.
...
From the driver and the bindings (`test/dom.test.ts`, `test/react/`, `test/vue/`, `test/svelte/`):
...
test/        Bun suite — invariants (1–7), closed (8–11), dom (12–15), react/ + vue/ + svelte/
             (16–17: morphicon.* SSR + mount.* client on happy-dom + svelte controller.test.ts +
             props-types.ts compile-time pins per binding), unit tests; bunfig.toml preloads
             test/register-client-dom.ts (happy-dom globals + rAF fake, see test/client-dom.ts)
             and test/svelte/loader.ts (.svelte server compile, client via ?client;
             test/svelte/harness.svelte drives prop swaps in the mount suite)
